...
#define CLK 10 // подключаем импульсы отсчета энкодера на 10-й цифровой пин (можно и на аналоговый - без разницы) - т.е. насколько сильно повернули энкодер
...
void setup() {
  ...
  enc.setFastTimeout(20); // таймаут на скорость isFast (по умолчанию 50) - чем больше сообщаемый аргумент, тем чувствительнее к прокрутке

  //Serial.begin(9600); // Раскомментировать, на случай предварительной настройки коэффициентов с использованием монитора порта или программ типа serialportplotter.exe
...
void loop() {
  ...
  switch (screenState) { // выбор нужного экрана (изначально screen было равно 0)
    case 0: // главное меню - дописать обработку отдельной кнопки СТАРТ/СТОП
      ...
      //digitalWrite(ledPin, ledOn); // включить светодиод при нажатии кнопки СТАРТ/СТОП (но он сразу потухнет или начнет мерцать, т.к. начнется регулирование) - раскомментить, если нужно
      break;
    ...
    case 5: // экран проведения регулирования
      ...
      // Раскомментировать следующие три строчки ниже, во время настройки коэффициентов при использовании программ для графиков в реальном времени, например serialportplotter.exe
      //Serial.println("$");
